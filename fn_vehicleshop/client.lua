                ...
                ESX.TriggerServerCallback('fn_vehicleshop:buy', function(canbuy)
                    if canbuy then
                        ESX.Game.SpawnVehicle(closest_car.model, Config.buy_point.pos, Config.buy_point.heading*1.0, function(vehicle)
                            ...
                            ESX.ShowNotification("You bought ~b~"..(closest_car.label~=nil and closest_car.label or GetDisplayNameFromVehicleModel(GetHashKey(closest_car.model))).."~s~ for "..Config["currency"].."~r~"..ESX.Math.GroupDigits(closest_car.price))
                        end)
                    end
